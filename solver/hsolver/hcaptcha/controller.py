import random, time; 
from math import ceil; from typing import Tuple
...
def mouse_curve(start_pos, end_pos, ln):
    ...
    control_1 = (start_pos[0] + random.choice((-1, 1)) * abs(ceil(end_pos[0]) - ceil(start_pos[0])) * 0.01 * 3,
                 start_pos[1] + random.choice((-1, 1)) * abs(ceil(end_pos[1]) - ceil(start_pos[1])) * 0.01 * 3)
    control_2 = (start_pos[0] + random.choice((-1, 1)) * abs(ceil(end_pos[0]) - ceil(start_pos[0])) * 0.01 * 3,
                 start_pos[1] + random.choice((-1, 1)) * abs(ceil(end_pos[1]) - ceil(start_pos[1])) * 0.01 * 3)
...
class MotionData:
    ...
    def addPoint(self, x, y):
        self.timestamp += random.randint(20, 40)
